...
- **Adaptive full-screen board.** The playfield sizes itself to your viewport: a bigger, wider grid on large monitors, taller on phones. A Small/Medium/Large board-size control sets the grid density (a bigger board uses smaller cells). Grid dimensions lock when a game starts, so resizing mid-play rescales instead of breaking the run.
